...
- **Créditos:** según `github.com/settings/billing`, usé **148.96 créditos** de Copilot en septiembre en total (incluye toda esta semana, del Día 1 al proyecto final). El único día que medí con un antes/después exacto fue el Día 2, donde gasté **47 créditos** (de 24 a 71 AIC en un solo día) — el resto los fui capturando por tarea individual en las capturas de cada sesión, no como total diario.

  Qué haría distinto para gastar menos: usar el **Plan B** (copiar la versión de referencia) más rápido cuando un primer intento del agente se desvía mucho de la spec, en vez de dejar que reintente — el primer intento de `overdue` en el Día 2 creó 11 archivos cuando la spec pedía 4, y seguir corrigiendo eso con más prompts habría costado más que simplemente descartar y volver a empezar. También exportaría sesiones completas (`/share file`) solo cuando de verdad necesito auditar el transcript completo, no por costumbre — algunas de esas exportaciones fueron más largas de lo necesario para lo que terminé usando de ellas.

- **Una cosa que el agente hizo mal:** en el Día 2, el primer intento de implementar `GET /tasks/overdue` con la skill creó **11 archivos** cuando la spec (`specs/overdue.md`) solo pedía 4 — agregó clases placeholder como `DataSeeder.java`, `JwtTokenFilter.java` y `SecurityConfig.java` que no venían al caso, algunas con comentarios que admitían su propia incertidumbre ("la implementación real se encuentra en... **si existe**"). Lo detectó el **Punto 1 del checklist** (Alcance: `git diff --stat main` debía mostrar solo la spec y los cuatro archivos que nombra) — el checklist comparó contra la spec real, no contra lo que el agente reportó haber hecho. Quedó corregido descartando el intento por completo (`git reset HEAD~1`) y repitiendo el mismo prompt desde cero: el segundo intento sí tocó exactamente los 5 archivos correctos y pasó los 6 puntos del checklist.
